        ...
        for event in pygame.event.get():
          if event.type == pygame.QUIT or event.type == pygame.locals.QUIT:
            ...
            if event.key == pygame.locals.K_UP:
              self.conn.sendto(bytes("uu", "utf-8"), (self.addr, self.serverport))
            elif event.key == pygame.locals.K_DOWN:
              self.conn.sendto(bytes("ud", "utf-8"), (self.addr, self.serverport))
            elif event.key == pygame.locals.K_LEFT:
              self.conn.sendto(bytes("ul", "utf-8"), (self.addr, self.serverport))
            elif event.key == pygame.locals.K_RIGHT:
              self.conn.sendto(bytes("ur", "utf-8"), (self.addr, self.serverport))
            pygame.event.clear(pygame.locals.KEYDOWN)

        pygame.display.update()
    finally:
      self.conn.sendto(bytes("d", "utf-8"), (self.addr, self.serverport))
